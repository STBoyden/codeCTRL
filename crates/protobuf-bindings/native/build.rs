...
fn main() {
    ...
    if !path.exists() {
        panic!(
            "Protos directory does not exist at \"{proto_path}\", please sync + update \
             the submodule."
        )
    }
    ...
    if let Ok(contents) = path.read_dir() {
        ...
        {
            panic!(
                "Protos directory does exist at \"{proto_path}\" but does not contain \
                 protobuf files. Please sync + update the submodule."
            );
        }
    }
