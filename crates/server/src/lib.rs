...
use codectrl_protobuf_bindings::{
	auth_service::{
		...
		GenerateTokenRequest, GenerateTokenRequestResult, LoginUrl, RevokeTokenRequestResult, Token,
		VerifyTokenRequest, VerifyTokenRequestResult,
	},
	data::Log,
	logs_service::{
		Connection, LogClientService, LogClientTrait, LogServerService, LogServerTrait, RequestResult,
		RequestStatus, ServerDetails,
	},
};
...
impl Service {
			...
			loop {
				...
				for mut connection in connections.write().await.iter_mut() {
					if connection.last_update.elapsed() >= Duration::new(5, 0) {
						let sent_logs =
							if let Ok(sent_log_ids) = serde_json::to_string(&connection.sent_log_ids) {
	...
	pub fn verify_log(log: &mut Log, remote_addr: Option<SocketAddr>, metadata: &MetadataMap) {
		...
		if let Some(censor_usernames) = CENSOR_USERNAMES.get() {
			if censor_usernames {
				log.file_name = Self::strip_username_from_path(&log.file_name).to_string();

				log.stack.iter_mut().for_each(|stack| {
					stack.file_path = Self::strip_username_from_path(&stack.file_path).to_string();
				});
			}
		}
...
impl LogServerTrait for Service {
	async fn register_client(&self, req: Request<()>) -> Result<Response<Connection>, Status> {
	...
	async fn get_server_details(&self, req: Request<()>) -> Result<Response<ServerDetails>, Status> {
	...
	async fn get_log(&self, connection: Request<Connection>) -> Result<Response<Log>, Status> {
		let remote_addr = connection.remote_addr().unwrap();
		let connection = connection.into_inner();
...
impl LogClientTrait for Service {
	async fn send_log(&self, request: Request<Log>) -> Result<Response<RequestResult>, Status> {
...
impl Authentication for Service {
	...
	async fn refresh_token(&self, _request: Request<Token>) -> Result<Response<Token>, Status> {
		todo!()
	}
...
pub async fn run_server(
	...
	let data_dir =
		if let Some(data_directory) = ProjectDirs::from("com", "stboyden", "codectrl-server") {
			data_directory.data_dir().to_owned()
		} else {
			Path::new(".codectrl-server").to_owned()
		};

	let requires_authentication = if let Some(requires_authentication) = requires_authentication {
